...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (s) { const p = JSON.parse(s); if (p?.ok) setSession(p); }
    setLoading(false);
  }, []);
  ...
  const fetchAllData = async (showRefreshToast = false) => {
    ...
    try {
      ...
      if(showRefreshToast) showToast("รีเฟรชแล้ว", "success");
      
      if (itemGroupModal) {
        const updatedGroups = buildTypeGroups(data.items || []);
        const freshGroup = updatedGroups.find(g => g.type === itemGroupModal.type);
        if (freshGroup) setItemGroupModal(freshGroup);
        else setItemGroupModal(null);
      }
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    if (password !== ADMIN_PASSWORD) return showToast("รหัสผ่านไม่ถูกต้อง", "error");
    const s = { ok: true, at: Date.now() };
              ...
              <tbody>
                {list.length === 0 ? <tr><td colSpan="6" className="text-center text-slate-400 py-8">ไม่พบข้อมูล</td></tr> : list.map(g => {
                  ...
                  if (g.consumable) {
                    ...
                    const avail = g.units.filter(u => u.status === "Available").length;
                    statusBadge = <span className="chip bg-blue-100 text-blue-700">ทั้งหมด {total} (ว่าง {avail})</span>;
                  }
                  return (
                    <tr key={g.type}>
          ...
          <div className="fixed inset-0 modal-bg flex items-center justify-center p-4 z-50">
            <div className="glass-dark rounded-3xl card-shadow max-w-md w-full p-6 animate-scale">
              ...
              <form onSubmit={async (e) => {
                ...
                for(let i=0; i<q; i++){
                  const num = s+i;
                  items.push({ itemId: `${t}_${String(num).padStart(3,"0")}`, type: t, name: n, color: c, imageUrl: imgUrl, status: "Available", number: num, consumable: false });
                }
                if(await callApi("items/bulk", "POST", {items})) setBulkModal(false);
              }} className="space-y-3">
